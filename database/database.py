...
class DatabaseNode(Node):
    ...
    def _save_url_content_local(self, url, result, completed_at):
        """Inserta o actualiza el contenido de una URL en la BD local.

        Args:
            url: URL a guardar
            result: Diccionario con el resultado (será serializado a JSON)
            completed_at: Timestamp de cuando se completó el scraping
        """
        ...
        with self.db_lock:
            ...
            self.db_cursor.execute(
                ...
                WHERE url = ?
            """,
                (content_json, completed_at, url),
            )
            self.db_conn.commit()

    def _count_url_replicas(self, url, node_filter=None):
        """Cuenta cuántos nodos distintos tienen registrada una URL en url_db_log.

        Args:
            url: URL a consultar
            node_filter: Lista opcional de node_ids a considerar. Si es None, cuenta todos.

        Returns:
            int: Número de réplicas
        """
        with self.db_lock:
    ...
    def _get_nodes_with_url(self, url):
        """Devuelve la lista de node_ids que tienen registrada una URL en url_db_log.

        Args:
            url: URL a consultar

        Returns:
            list[str]: Lista de node_ids
        """
        with self.db_lock:
            self.db_cursor.execute(
                "SELECT node_id FROM url_db_log WHERE url = ?", (url,)
            )
            return [row[0] for row in self.db_cursor.fetchall()]

    def _get_subordinate_connection(self, node_id):
        """Busca y devuelve la conexión de un subordinado por node_id (coincidencia parcial).

        Args:
            node_id: ID del subordinado (puede ser parcial)

        Returns:
            NodeConnection | None: Conexión encontrada o None
        """
    ...
    def _build_response_message(self, msg_type_key, data_dict):
        """Construye un dict de respuesta estándar con type/sender_id/timestamp/data.

        Args:
            msg_type_key: Clave del tipo en MessageProtocol.MESSAGE_TYPES
            data_dict: Datos a incluir en el campo 'data'

        Returns:
            dict: Mensaje listo para enviar
        """
        return {
            "type": MessageProtocol.MESSAGE_TYPES[msg_type_key],
    ...
    def reassign_tasks_from_subordinate(self, node_id):
        ...
        """
        logging.info(
            f"[REREPLICATE] reassign_tasks_from_subordinate llamado para: {node_id}"
        )
        ...
        if not self.i_am_boss:
            logging.info(
                f"[REREPLICATE] No soy jefe, ignorando desconexión de {node_id}"
            )
            return

        logging.info(
            f"[REREPLICATE] Soy jefe BD, iniciando re-replicación para: {node_id}"
        )
        ...
        try:
            ...
            if not self.db_conn:
                logging.error(
                    f"[REREPLICATE] Base de datos no inicializada, no se puede re-replicar"
                )
                return

            # Simplemente llamar la re-replicación con el node_id
            logging.info(
                f"[REREPLICATE] Iniciando re-replicación para node_id: {node_id}"
            )
            self._handle_subordinate_disconnection(node_id)

        except Exception as e:
            logging.error(
                f"[REREPLICATE] Error iniciando re-replicación para {node_id}: {e}"
            )
            ...
            try:
                ...
                for node_id in db_subordinates:
                    if node_id not in connected_node_ids:
                        logging.warning(
                            f"Subordinado {node_id} está desconectado, iniciando re-replicación"
                        )
                        self._handle_subordinate_disconnection(node_id)
    ...
    def _handle_subordinate_disconnection(self, disconnected_node_id):
        ...
        try:
            ...
            if not affected_urls:
                logging.info(
                    f"No hay URLs afectadas por desconexión de {disconnected_node_id}"
                )
                return

            logging.info(
                f"Procesando {len(affected_urls)} URLs afectadas por desconexión"
            )
            ...
            logging.info(
                f"Registros de {disconnected_node_id} eliminados de url_db_log"
            )

            # Para cada URL afectada, verificar y re-replicar si es necesario
            for url in affected_urls:
                ...
                if connected_nodes:
                    current_replicas = self._count_url_replicas(url, connected_nodes)
                else:
                    current_replicas = 0

                logging.info(
                    f"URL {url} ahora tiene {current_replicas} réplicas (era 1 más antes)"
                )

                # Si necesita más réplicas, intentar re-replicar
                if current_replicas < TARGET_REPLICAS:
                    logging.info(f"URL {url} necesita re-replicación")
                    self._check_and_rereplicate_url(url)

            logging.info(
                f"Desconexión de {disconnected_node_id} procesada completamente"
            )
    ...
    def _on_subordinate_inherited_from_conflict(self, subordinate_node_id):
        ...
        """
        logging.info(
            f"🔄 Subordinado BD heredado: {subordinate_node_id}. Iniciando consolidación de URLs..."
        )

        # Dar un breve momento para que el subordinado se estabilice
        threading.Timer(2.0, self._consolidate_duplicate_urls).start()

    def _check_and_rereplicate_url(self, url, exclude_node_id=None):
        """
        ...
        try:
            ...
            logging.info(
                f"URL {url} tiene {current_replicas} réplicas, necesita {needed_replicas} más"
            )
            if exclude_node_id:
            ...
            if not source_row:
                logging.error(
                    f"No hay réplicas activas para URL {url}, no se puede re-replicar"
                )
                return
            ...
            with self.subordinates_lock:
                connected_node_ids = [
                    nid for nid in self.subordinates.keys() if nid != exclude_node_id
                ]

            logging.info(
                f"[REREPLICATE] Subordinados conectados (excluyendo {exclude_node_id}): {connected_node_ids}"
            )

            nodes_with_url = self._get_nodes_with_url(url)

            available_subordinates = [
                nid for nid in connected_nodes if nid not in nodes_with_url
            ]

            if not available_subordinates:
                logging.warning(
                    f"[REREPLICATE] No hay subordinados disponibles para re-replicar URL {url}. Conectados: {len(connected_node_ids)}, Disponibles sin URL: 0"
                )
                return

            # Seleccionar subordinados para re-replicar (hasta completar TARGET_REPLICAS réplicas)
            random.shuffle(available_subordinates)
            targets = available_subordinates[:needed_replicas]

            logging.info(
                f"[REREPLICATE] Targets seleccionados para URL {url}: {targets}"
            )

            # Solicitar contenido al subordinado fuente y enviarlo a los targets
            self._request_and_replicate_content(url, source_node_id, targets)

        except Exception as e:
            logging.error(f"Error verificando/re-replicando URL {url}: {e}")

            traceback.print_exc()

    def _request_and_replicate_content(self, url, source_node_id, target_subordinates):
        """
        ...
        try:
            ...
            source_conn = self._get_subordinate_connection(source_node_id)

            if not source_conn:
                logging.error(
                    f"No hay conexión con subordinado fuente {source_node_id}"
                )
                return
            ...
            for node_id in target_subordinates:
                ...
                if "-" in node_id and ":" in node_id:
                    ...
                    targets_info.append({"node_id": node_id, "ip": ip})
            ...
            logging.info(
                f"Solicitando a {source_node_id} que replique URL {url} a {len(targets_info)} subordinados"
            )
            ...
            new_count = self._count_url_replicas(url)
    ...
    def _self_replicate_content(self, url, targets):
        ...
        """
        logging.info(
            f"REPLICATE_CONTENT recibido: replicar {url} a {len(targets)} subordinados"
        )
        ...
        try:
            ...
            with self.db_lock:
                self.db_cursor.execute(
                    "SELECT content, scrapped_at FROM urls WHERE url = ?", (url,)
                )
            ...
            for target_info in targets:
                ...
                target_conn = self._get_subordinate_connection(target_node_id)
                ...
                if target_conn:
                    ...
                    try:
                        ...
                        logging.info(
                            f"Contenido replicado a subordinado {target_node_id}"
                        )
                    except Exception as e:
                        logging.error(
                            f"Error enviando contenido a {target_node_id}: {e}"
                        )
                else:
                    logging.warning(
                        f"No hay conexión con subordinado destino {target_node_id}"
                    )
    ...
    def _recover_metadata_from_subordinates(self):
        ...
        with self.db_lock:
            self.db_cursor.execute(
                "SELECT url, content, scrapped_at FROM urls WHERE content IS NOT NULL"
            )
            own_urls = [
                {"url": r[0], "content": r[1], "scrapped_at": r[2]}
    ...
    def _handle_request_url_inventory(self, node_connection, message):
        ...
        """
        logging.info(
            "REQUEST_URL_INVENTORY recibido del nuevo jefe, preparando inventario..."
        )
        ...
        try:
            ...
            with self.db_lock:
                self.db_cursor.execute("""
                    SELECT url, content, scrapped_at
                    FROM urls
                    WHERE content IS NOT NULL
                """)
                urls = [
                    {"url": row[0], "content": row[1], "scrapped_at": row[2]}
    ...
    def _handle_report_url_inventory(self, node_connection, message):
        ...
        logging.info(
            f"REPORT_URL_INVENTORY recibido de {reporter_node_id}: {len(urls)} URLs"
        )
        ...
        if hasattr(self, "inventory_responses"):
            ...
            all_responded = all(
                v is not None for v in self.inventory_responses.values()
            )
        ...
        try:
            ...
            for node_id, urls in self.inventory_responses.items():
                ...
                for url_info in urls:
                    ...
                    # Contar réplicas
                    url_replica_count[url] = url_replica_count.get(url, 0) + 1

            self.db_conn.commit()
            logging.info(
                f"Tablas urls y url_db_log reconstruidas con {len(url_replica_count)} URLs únicas"
            )
        ...
        try:
            ...
            with self.db_lock:
                self.db_cursor.execute("""
                    ...
                    WHERE content IS NOT NULL
                """)
                own_urls = [row[0] for row in self.db_cursor.fetchall()]
            ...
            with self.db_lock:
                for url in own_urls:
                    ...
                    if self.db_cursor.rowcount > 0:
                        ...
                        logging.info(
                            f"URL {url} registrada, ahora tiene {new_count} réplicas"
                        )
                self.db_conn.commit()
    ...
    def _consolidate_duplicate_urls(self):
        ...
        try:
            ...
            for url, versions in url_versions.items():
                ...
                logging.warning(
                    f"   Versión más reciente: {newest_date} (nodo: {newest_node})"
                )
                ...
                for node_id, scrapped_at, content in versions[1:]:
                    if scrapped_at != newest_date or content != newest_content:
                        updates_needed += 1
                        logging.info(
                            f"   → Actualizando {node_id} con versión más reciente"
                        )

                        # Enviar UPDATE_URL_CONTENT a ese nodo
                        if node_id == self.node_id:
                            # Actualizar mi propia BD
                            self._update_own_url_content(
                                url, newest_content, newest_date
                            )
                        else:
            ...
            with self.db_lock:
                for url, versions in url_versions.items():
                    ...
                    if best_content is None:
                        continue
                    self.db_cursor.execute(
                        "SELECT scrapped_at FROM urls WHERE url = ?", (url,)
                    )
                    ...
                    if row is None:
                        ...
                        self.db_cursor.execute(
                            "INSERT OR IGNORE INTO url_db_log (url, node_id) VALUES (?, ?)",
                            (url, self.node_id),
                        )
                        synced += 1
                        ...
                        if existing_date is None or (
                            best_date and best_date > existing_date
                        ):
                            self.db_cursor.execute(
                                "UPDATE urls SET content = ?, scrapped_at = ? WHERE url = ?",
                                (best_content, best_date, url),
                            )
                            synced += 1
                self.db_conn.commit()
            logging.info(
                f"Sincronización completada: {synced} URLs actualizadas/importadas al jefe"
            )
        ...
        try:
            ...
            for url, versions in url_versions.items():
                ...
                if current_replicas > target_replicas:
                    ...
                    logging.warning(
                        f"⚠️  URL {url} tiene {current_replicas} réplicas (exceso: {excess})"
                    )
                    ...
                    if not available:
                        logging.warning(
                            f"URL {url} tiene solo {current_replicas} réplicas y no hay nodos disponibles para más"
                        )
                        continue

                    targets = random.sample(available, min(needed, len(available)))
                    logging.info(
                        f"URL {url} tiene {current_replicas} réplicas, replicando a {len(targets)} nodos más"
                    )
                    ...
                    for target_node_id in targets:
                        if target_node_id == self.node_id:
                            ...
                            source_node_id = (
                                nodes_with_url[0] if nodes_with_url else None
                            )
                            if source_node_id:
                                logging.info(
                                    f"   → Jefe solicitando contenido de {source_node_id} para URL {url}"
                                )
                                self._request_and_replicate_content(
                                    url, source_node_id, []
                                )
                        else:
                            with self.subordinates_lock:
                            ...
                            if target_conn and content_json:
                                ...
                                replicate_msg = self._create_message(
                                    MessageProtocol.MESSAGE_TYPES[
                                        "SAVE_DATA_NO_LEADER"
                                    ],
                                    {
                                        "url": url,
                                        "result": result,
                                        "task_id": f"balance-{url}",
                                    },
                                )
                                ...
                                replicas_added += 1
                                logging.info(
                                    f"   → URL {url} replicada a {target_node_id}"
                                )
    ...
    def _delete_own_url_content(self, url):
        ...
        try:
            with self.db_lock:
                ...
                self.db_cursor.execute(
                    """
                    SELECT COUNT(*) FROM url_db_log WHERE url = ?
                """,
                    (url,),
                )
                ...
                if remaining_replicas == 0:
                    ...
                    self.db_cursor.execute("DELETE FROM urls WHERE url = ?", (url,))
                    logging.info(
                        f"✓ URL {url} eliminada completamente (última réplica)"
                    )
                else:
                    logging.info(
                        f"✓ Réplica local de {url} eliminada ({remaining_replicas} réplicas restantes)"
                    )
    ...
    def _send_url_deletion_to_subordinate(self, node_id, url):
        ...
        try:
            ...
            if not conn:
                logging.error(
                    f"Subordinado {node_id} no encontrado para eliminar {url}"
                )
                return

            delete_message = self._create_message(
                MessageProtocol.MESSAGE_TYPES["DELETE_URL_CONTENT"], {"url": url}
            )
    ...
    def _send_url_update_to_subordinate(self, node_id, url, content, scrapped_at):
        ...
        try:
            ...
            if not conn:
                logging.error(
                    f"Subordinado {node_id} no encontrado para actualizar {url}"
                )
                return

            update_message = self._create_message(
                MessageProtocol.MESSAGE_TYPES["UPDATE_URL_CONTENT"],
                {"url": url, "content": content, "scrapped_at": scrapped_at},
            )
    ...
    def _redistribute_own_urls(self):
        ...
        try:
            ...
            if not own_urls:
                logging.info(
                    "Este nodo no tenía URLs almacenadas, no se necesita redistribución"
                )
                return

            logging.info(f"Redistribuyendo {len(own_urls)} URLs que tenía este nodo...")

            for url, content_json, scrapped_at in own_urls:
                # Contar cuántas réplicas hay en subordinados
                with self.db_lock:
                    self.db_cursor.execute(
                        """
                        SELECT COUNT(DISTINCT node_id)
                ...
                if needed_replicas <= 0:
                    logging.info(
                        f"URL {url} ya tiene {current_replicas} réplicas, no se necesita redistribuir"
                    )
                    continue
                ...
                available_subordinates = [
                    nid for nid in connected_node_ids if nid not in nodes_with_url
                ]

                if not available_subordinates:
                    logging.warning(
                        f"No hay subordinados disponibles para redistribuir URL {url}"
                    )
                    continue
                ...
                logging.info(
                    f"Redistribuyendo URL {url} a {len(targets)} subordinados: {targets}"
                )
                ...
                with self.db_lock:
                    for node_id in targets:
                        ...
                        if target_conn:
                            ...
                            logging.warning(
                                f"No hay conexión con subordinado {node_id}"
                            )
    ...
    def add_subordinate(self, node_ip, existing_socket=None):
        ...
        if self.i_am_boss:
            ...
            logging.info(
                f"[NEW_SUB] Nuevo subordinado BD {node_id} agregado, iniciando replicación automática"
            )

            # Ejecutar replicación en un thread separado para no bloquear
            threading.Thread(
                target=self._replicate_to_new_subordinate, args=(node_id,), daemon=True
            ).start()
    ...
    def _replicate_to_new_subordinate(self, new_subordinate_id):
        ...
        try:
            ...
            logging.info(
                f"[REPLICATE_NEW_SUB] Iniciando replicación hacia nuevo subordinado {new_subordinate_id}"
            )
            ...
            with self.db_lock:
                self.db_cursor.execute(
                    ...
                    FROM urls u
                    WHERE (SELECT COUNT(DISTINCT l.node_id) FROM url_db_log l WHERE l.url = u.url) < ?
                """,
                    (TARGET_REPLICAS,),
                )
            ...
            if not urls_needing_replication:
                logging.info(
                    f"[REPLICATE_NEW_SUB] No hay URLs que necesiten replicación"
                )
                return

            logging.info(
                f"[REPLICATE_NEW_SUB] Encontradas {len(urls_needing_replication)} URLs que necesitan replicación"
            )

            # Para cada URL que necesita replicación
            replicated_count = 0
            for (url,) in urls_needing_replication:
                try:
                    ...
                    if not content_row:
                        logging.warning(
                            f"[REPLICATE_NEW_SUB] No se encontró contenido para url={url}"
                        )
                        continue
                    ...
                    with self.subordinates_lock:
                        if new_subordinate_id in self.subordinates:
                            ...
                            logging.info(
                                f"[REPLICATE_NEW_SUB] URL {url} replicada a {new_subordinate_id}"
                            )
                        else:
                            logging.warning(
                                f"[REPLICATE_NEW_SUB] Subordinado {new_subordinate_id} ya no está conectado"
                            )
                            break

                except Exception as e:
                    logging.error(
                        f"[REPLICATE_NEW_SUB] Error replicando url={url}: {e}"
                    )
                    continue

            logging.info(
                f"[REPLICATE_NEW_SUB] Replicación completada: {replicated_count} URLs replicadas a {new_subordinate_id}"
            )
    ...
    def _register_bd_handlers(self):
        ...
        self.add_persistent_message_handler(
            MessageProtocol.MESSAGE_TYPES["BD_QUERY"], self._url_query_leader
        )

        # Handler para query de url entre nodos BD (líder -> subordinado)
        self.add_persistent_message_handler(
            MessageProtocol.MESSAGE_TYPES["URL_QUERY"], self._url_query_noleader
        )

        # Handler para guardar datos desde Scrapper (solo líder)
        self.add_persistent_message_handler(
            MessageProtocol.MESSAGE_TYPES["SAVE_DATA"], self._recive_task_result
        )
        ...
        self.add_persistent_message_handler(
            MessageProtocol.MESSAGE_TYPES["LIST_TABLES"], self._handle_list_tables
        )

        # Handler para obtener datos paginados de tabla
        self.add_persistent_message_handler(
            MessageProtocol.MESSAGE_TYPES["GET_TABLE_DATA"], self._handle_get_table_data
        )
    ...
    def _register_url_in_subordinates(self, url, node_ids):
        ...
        try:
            # Insertar URL en tabla urls si no existe y registrar en url_db_log
            with self.db_lock:
                self.db_cursor.execute(
                    "INSERT OR IGNORE INTO urls (url) VALUES (?)", (url,)
                )
            ...
            logging.info(
                f"Líder BD registró URL {url} en {len(node_ids)} subordinados: {node_ids} (total réplicas: {actual_replicas})"
            )
        ...
        try:
            with self.db_lock:
                ...
                if result is None:
                    ...
                    logging.info(
                        f"URL {url} NO encontrada en BD, responder negativo al Router"
                    )
                    self._send_bd_query_response(node_connection, task_id, found=False)
                    return

                logging.info(f"URL {url} encontrada en BD")
            ...
            if len(databases_with_content) == 0:
                ...
                logging.warning(
                    f"URL {url} registrada pero sin contenido en subordinados"
                )
                self._send_bd_query_response(node_connection, task_id, found=False)
                return
            ...
            if subordinate_id == self.node_id:
                ...
                logging.info(
                    f"El contenido está en el jefe mismo ({self.node_id}), leyendo localmente"
                )
                with self.db_lock:
                    self.db_cursor.execute(
                ...
                if content_row and content_row[0]:
                    ...
                    self._send_bd_query_response(
                        node_connection, task_id, found=True, result=result
                    )
                else:
                    logging.warning(
                        f"Contenido vacío para url={url}, responder negativo"
                    )
                    self._send_bd_query_response(node_connection, task_id, found=False)
                return

            logging.info(
                f"Solicitando contenido de URL al subordinado {subordinate_id}"
            )

            # Buscar conexión con el subordinado
            subordinate_conn = self._get_subordinate_connection(subordinate_id)
            ...
            if subordinate_conn:
                ...
                logging.info(
                    f"URL_QUERY enviada al subordinado {subordinate_id} para URL {url}"
                )
            else:
                # No hay conexión con el subordinado que tiene el contenido
                logging.warning(
                    f"No hay conexión con subordinado {subordinate_id}, responder negativo"
                )
                self._send_bd_query_response(node_connection, task_id, found=False)
    ...
    def _send_bd_query_response(self, router_connection, task_id, found, result=None):
        ...
        response = self._create_message(
            ...
            {"task_id": task_id, "found": found, "result": result if result else {}},
        )

        try:
            router_connection.send_message(response)
            logging.info(
                f"BD_QUERY_RESPONSE enviada al Router: task={task_id}, found={found}"
            )
        except Exception as e:
    ...
    def _url_query_noleader(self, node_connection, message):
        ...
        try:
            with self.db_lock:
                ...
                self.db_cursor.execute("SELECT content FROM urls WHERE url = ?", (url,))
                content_row = self.db_cursor.fetchone()

                if content_row is None:
                    logging.error(
                        f"URL {url} no encontrada en tabla urls del subordinado"
                    )
    ...
    def _send_bd_query_response_to_router(self, task_id, found, result=None):
        ...
        try:
            ...
            try:
                ...
                logging.info(
                    f"Subordinado BD envió BD_QUERY_RESPONSE al Router {router_ip}:{router_port}: task={task_id}, found={found}"
                )
            finally:
                sock.close()
    ...
    def _notify_router_of_error_result(self, task_id, result):
        ...
        if not router_profile or not router_profile.is_connected():
            logging.warning(
                f"No hay conexión con Router para notificar error de task {task_id}"
            )
            return
    ...
    def _recive_task_result(self, node_connection, message):
        ...
        if result.get("status") == "error":
            logging.warning(
                f"Resultado con error para task {task_id}: {result.get('error')}"
            )
        ...
        try:
            self._save_url_content_local(url, result, completed_at)
            logging.info(f"Líder BD guardó contenido de {url}")
        ...
        if len(subordinados) == 0:
            logging.warning(
                f"No hay subordinados BD. Solo el líder tiene el contenido."
            )
            return
    ...
    def _recive_task_result_no_leader(self, node_connection, message):
        ...
        try:
            self._save_url_content_local(url, result, completed_at)
            logging.info(f"Subordinado BD guardó contenido de {url}")
        except Exception as e:
    ...
    def _connect_to_external_bosses(self):
        ...
        threading.Thread(
            target=self._periodic_boss_search, args=("router",), daemon=True
        ).start()
    ...
    def _find_boss_in_list(self, ip_list, node_type):
        ...
        logging.debug(
            f"Buscando jefe {node_type} en lista de {len(ip_list)} IPs: {ip_list}"
        )
        ...
        for ip in ip_list:
            ...
            msg = self._create_message(
                MessageProtocol.MESSAGE_TYPES["IDENTIFICATION"], {"is_temporary": True}
            )
    ...
    def _connect_to_boss(self, node_type, boss_ip):
        ...
        try:
            ...
            if connection.connect():
                logging.debug(
                    f"Socket conectado exitosamente con {node_type} en {boss_ip}"
                )
                ...
                logging.debug(
                    f"BossProfile actualizado. is_connected={boss_profile.is_connected()}"
                )
                ...
                if self.i_am_boss:
                    ...
                    logging.info(
                        f"Información de jefe {node_type} replicada a subordinados"
                    )
            else:
    ...
    def _handle_list_tables(self, node_connection, message):
        ...
        try:
            ...
            response = self._build_response_message(
                ...
                {"request_id": request_id, "tables": tables, "success": True},
            )

            node_connection.send_message(response)
            logging.info(
                f"Lista de {len(tables)} tablas enviada a {node_connection.node_id}"
            )
    ...
    def _handle_get_table_data(self, node_connection, message):
        ...
        try:
            ...
            node_connection.send_message(response)
            logging.info(
                f"Datos de tabla {table_name} (página {page}/{total_pages}) enviados a {node_connection.node_id}"
            )
    ...
    def _handle_export_all_data(self, node_connection, message):
        ...
        try:
            ...
            cursor.execute("""
                SELECT url, content, firstseen, scrapped_at
                FROM urls
            ...
            node_connection.send_message(response)
            logging.info(
                f"✓ Exportación completada: {len(urls_data)} URLs enviadas a {node_connection.node_id}"
            )
    ...
    def _handle_update_url_content(self, node_connection, message):
        ...
        try:
            with self.db_lock:
                ...
                if affected > 0:
                    logging.info(
                        f"✓ Contenido de {url} actualizado exitosamente (fecha: {scrapped_at})"
                    )
                else:
                    logging.warning(
                        f"URL {url} no encontrada en la BD, insertándola..."
                    )
    ...
    def _handle_delete_url_content(self, node_connection, message):
        ...
        try:
            with self.db_lock:
                ...
                if affected > 0:
                    logging.info(
                        f"✓ URL {url} eliminada exitosamente (balanceo de réplicas)"
                    )
                else:
