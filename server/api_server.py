...
class ServerConnector:
    """Conector para comunicarse con el servidor principal via TCP"""
    ...
    def connect(self):
        ...
        try:
            ...
            logging.info(f"Conectado al servidor: {welcome_msg}")
            # Quitar timeout una vez conectado
            self.socket.settimeout(None)
            return True
            
        except Exception as e:
            logging.warning(f"No se pudo conectar al servidor principal: {e}")
            self.connected = False
            if self.socket:
                try:
                    self.socket.close()
    ...
    def send_message(self, message_dict):
        """Envía un mensaje al servidor usando el protocolo existente"""
    ...
    def send_scrape_command(self, url):
        """Envía comando de scraping al servidor"""
        command_msg = {
            'type': 'command',
    ...
    def get_server_logs(self):
        ...
        try:
            ...
            logging.error(f"Error obteniendo logs del servidor: {e}")
            self.connected = False
            return None
    
    def get_system_status(self):
        """Obtiene el estado del sistema del servidor principal"""
        if not self.connected:
            if not self.connect():
                return None
        ...
        try:
            ...
            logging.error(f"Error obteniendo estado del servidor: {e}")
            self.connected = False
            return None
...
def create_scrape_task():
    ...
    try:
        ...
        if success:
            return jsonify({
                'status': 'success',
                'message': 'Tarea de scraping enviada al servidor',
                'url': url,
                'timestamp': datetime.now().isoformat()
            }), 201
        else:
            ...
            if not server_connector.connected:
                error_msg = 'Servidor principal no disponible. Verificar que esté corriendo en puerto 8080.'
            else:
            ...
            return jsonify({
                ...
                'url': url,
                'server_connected': server_connector.connected,
                'suggestion': 'Asegúrate de que el servidor principal esté corriendo: python3 server/server.py'
            }), 503
...
def get_api_status():
    ...
    api_status = {
        ...
        'server_connection': {
            ...
            'host': server_connector.server_host,
            'port': server_connector.server_port
        },
        'timestamp': datetime.now().isoformat()
    }
    
    # Intentar obtener estado del servidor principal
    server_status = server_connector.get_system_status()
    if server_status:
...
def get_logs():
    ...
    try:
        ...
        if log_type in ['all', 'server']:
            server_logs = server_connector.get_server_logs()
            if server_logs:
                logs.extend([{
                    'source': 'server',
...
def init_server_connection():
    """Inicializa la conexión con el servidor principal de forma no bloqueante"""
    def try_connect():
        if server_connector.connect():
            logging.info("✅ Conexión inicial con servidor principal establecida")
        else:
            logging.info("⚠️  Servidor principal no disponible (se intentará conectar cuando sea necesario)")
...
if __name__ == '__main__':
    ...
    logging.info(f"Conectando a servidor principal en {server_connector.server_host}:{server_connector.server_port}")
    
    # Intentar conectar al servidor principal de forma no bloqueante
